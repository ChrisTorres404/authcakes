...
describe('Enterprise Security E2E Tests', () => {
  ...
  describe('Authentication Security', () => {
    ...
    it('should prevent timing attacks on user enumeration', async () => {
      ...
      const avgExisting =
        timings.existing.reduce((a, b) => a + b) / timings.existing.length;
      const avgNonExisting =
        timings.nonExisting.reduce((a, b) => a + b) /
        timings.nonExisting.length;
  ...
  describe('Session Security', () => {
    ...
    it('should implement session fixation protection', async () => {
      ...
      await request(app.getHttpServer())
        ...
        .set(
          'Cookie',
          Array.isArray(initialCookies) ? initialCookies : [initialCookies],
        )
        .send({
          email,
          password: 'Test1234!',
      ...
      const sessionIdChanged =
        !Array.isArray(loginCookies) ||
        !loginCookies.some(
          (cookie) =>
            Array.isArray(initialCookies) &&
            initialCookies.some((initial) => cookie === initial),
        );
  ...
  describe('API Versioning & Deprecation', () => {
    it('should handle API versioning properly', async () => {
      ...
      const v1Res = await request(app.getHttpServer()).get('/api/v1/health');
